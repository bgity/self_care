...
SelfcareApp.controller('DietController', function ($rootScope, $scope, $state, $filter, $timeout, FoodMaster, $http, UomMaster, DietFactory, FoodPref, createDialog,filterFilter,$stateParams) {
	...
	/* program type */
	$scope.programeType = [{'id':1,'name':'Weight Loss'},{'id':2,'name':'Weight Gain'},{'id':3,'name':'NA'}];
	...
	$scope.checkAll = function () {
		...
		if($scope.checkedIndexs.length > 0 && $scope.formData.planid > 0) {
	...
	$scope.checkedRow = function (id) { 
		if ($scope.checkedIndexs.indexOf(id) === -1) {
			...
			// delete $scope.formData.exercise_new[id];
	...
	$scope.$on("update_controller", function(event, data){
		...
		$scope.checkedRow = function (id) {
			if ($scope.mealoptsn.indexOf(id) === -1) { //alert('hi')
				...
				// add exercise
				//$scope.formData.exercise_new[id] = {'items':[{exercise_name:'',duration:'',ex_def:''}]};
