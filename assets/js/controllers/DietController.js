...
SelfcareApp.controller('DietController', function ($rootScope, $scope, $state, $filter, $timeout, FoodMaster, $http, UomMaster, DietFactory, FoodPref, createDialog,filterFilter,$stateParams) {
	...
	/*fat ideal range */
	$scope.fat_ideal_range = [{id:1, age_group:'9 - 19', range: '12 - 16',gender: 'Male'}, {id:2, age_group:'20 - 39', range: '8 - 20',gender: 'Male'},{id:3, age_group:'40 - 59', range: '11 - 22',gender: 'Male'},{id:4, age_group:'60 - 79', range: '13 - 25',gender: 'Male'},{id:5, age_group:'9 - 19', range: '21 - 24',gender: 'Female'}, {id:6, age_group:'20 - 39', range: '21 - 33',gender: 'Female'},{id:7, age_group:'40 - 59', range: '23 - 34',gender: 'Female'},{id:8, age_group:'60 - 79', range: '24 - 36',gender: 'Female'}];
	...
	$scope.ageRelUpdate = function() { 
		...
		// update ideal fat range
		
		if($scope.formData.age >=9 && $scope.formData.age <= 19 )
		{
		   if($scope.formData.gender.value == 'Male')
			    $scope.formData.fat_ideal_range = $scope.fat_ideal_range[0].range; 
			else
				$scope.formData.fat_ideal_range = $scope.fat_ideal_range[4].range;
		}
		if($scope.formData.age >=20 && $scope.formData.age <= 39 )
		{
		   if($scope.formData.gender.value == 'Male')
			    $scope.formData.fat_ideal_range = $scope.fat_ideal_range[1].range; 
			else
				$scope.formData.fat_ideal_range = $scope.fat_ideal_range[5].range;
		}
		if($scope.formData.age >=40 && $scope.formData.age <= 59 )
		{
		   if($scope.formData.gender.value == 'Male')
			    $scope.formData.fat_ideal_range = $scope.fat_ideal_range[2].range; 
			else
				$scope.formData.fat_ideal_range = $scope.fat_ideal_range[6].range;
		}
		if($scope.formData.age >=60 && $scope.formData.age <= 79 )
		{
		   if($scope.formData.gender.value == 'Male')
			    $scope.formData.fat_ideal_range = $scope.fat_ideal_range[3].range; 
			else
				$scope.formData.fat_ideal_range = $scope.fat_ideal_range[7].range;
		}
	}
	...
	// Delete food item
	$scope.delOpItem = function(mealid, option, col,types) {
		
		console.log('#col'+mealid+'_'+option+'_'+col+'_'+types )
		...
		{       console.log('nodraft')
		       ...
		       $scope.formData.meals  = $scope.meals;				
				...
				for(var i=0; i < data.mealtimes.length; i++) 
				{
					...
					$scope.formData.fooditems[data.mealtimes[i].id].notes = data.mealtimes[i].notes;
				}
		}
		else
		{  console.log('draft')
	...
	$scope.$on('keydown', function (evt, obj, key) {
