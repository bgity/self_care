SelfcareApp.controller('DietChartController', function ($rootScope, $scope, $state, DietFactory, $stateParams, $http, $q, createDialog) {
   ...
   //console.log($stateParams.plan_id);
	...
	$scope.sendMail = function(type, clientId, planId) { 
		  createDialog({
            id: 'selectPrint',
            ...
            footerTemplate: ' <div style="width:100%; padding:5px 5px 25px 5px; text-align:center;">' +
			  ...
			  success: {label: 'Email', fn: function () {
                    var exercise_stat = $('#exercise').is(':checked');
					var guideline_stat = $('#guideline').is(':checked');				  
					DietFactory.sendMailChart(clientId,planId,exercise_stat,guideline_stat,type).success(function (data) {
					  console.log('Email sent');
					});
				}}
        }
	...
	// uncheck disease guidelines
	$scope.changeRecomend = function(id){
	    ...
	    data = 'id='+id+'&chk='+chk;
		$http.post('diet/removeRecomend',data).success(function(data){
				console.log(data)
		});
	...
	$scope.openPrint = function (type, clientId, planId) {
        ...
        createDialog({
            ...
            success: {label: 'Save', fn: function () {
					var exercise_stat = $('#exercise').is(':checked');
					var guideline_stat = $('#guideline').is(':checked');
					var downloadPath = 'diet/downloadPDF/'+clientId+'/'+planId+'/'+exercise_stat+'/'+guideline_stat+'/'+type;
					if(type=='Download')
					   window.open(downloadPath, '_self', '');
				    else
                       window.open(downloadPath,'','width=900,height=700,resizable=1');						
                }}
        }
        );
    };
