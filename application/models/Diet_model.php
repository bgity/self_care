...
class Diet_model extends CI_Model {
	...
	public function addPlan() {
		...
		if($post['planid']>0 && $post['copy_plan']==0)
		{
            ...
            if(isset($post['target_date']) && $post['target_date']!='00-00-0000')
			{ 
			...
			$arr_planProfile['target_weight_in_kg'] = (isset($post['target_weight_in_kg'])?$post['target_weight_in_kg']:'');
			$arr_planProfile['target_fat'] = (isset($post['target_fat'])?$post['target_fat']:'');
			$arr_planProfile['fat_ideal_range'] = (isset($post['fat_ideal_range'])?$post['fat_ideal_range']:'');
			$arr_planProfile['started_fat'] = (isset($post['started_fat'])?$post['started_fat']:'');
		...
		//	}
			...
			//echo '<pre>';print_r($foodItems);
			$meals = $post['meals'];
			foreach($foodItems as $mkey=>$value) {
			  if($meals[$mkey-1]['checked']=='true')
			  {
		...
		// Condition added for checking partial "Save" form from Meal Exchange page
		//if($post['selectedTab'] != 'meal')
		//{
			$arr_planMeals = array();
			$arr_planMealOptions = array();
			$arr_planMealItems = array();
	...
	//	echo $clientId.'==='.$planId;exit;
		...
		$arr_programme_type = array(1=>'Weight Gain', 2=>'Weight Loss', 3=>'Neutral');
		...
		foreach($result['meals'] as $key=>$opt_value) {
			...
			foreach($opt_result as $ikey=>$ivalue) {
				...
				foreach($item_result as $kk=>$vv)
				{
					...
					if($vv['macro_nut']=='CARBS')
					{
						$count_carb++;
						$item_result[$kk]['last_row_carb'] = $count_carb;
					}
					if($vv['macro_nut']=='PROTEIN')
					{
						$count_prot++;
						$item_result[$kk]['last_row_prot'] = $count_prot;
					}
					if($vv['macro_nut']=='FATS')
					{
						$count_fat++;
						$item_result[$kk]['last_row_fat'] = $count_fat;
					}
					if($vv['macro_nut']=='FIBRE')
					{
						$count_fibre++;
						$item_result[$kk]['last_row_fibre'] = $count_fibre;
					}
				...
				$item_result['count_carb'] = $count_carb;
				$item_result['count_prot'] = $count_prot;
				$item_result['count_fat'] = $count_fat;
				$item_result['count_fibre'] = $count_fibre;
				//$item_result['option_count'] = count($item_result);
		...
		// echo '<pre>';print_r($result['meals']);exit;
		// get Recommended intake
		$arr_recommIntake = $this->getPlanProfile($planId);
		...
		$arr_targetDetails = $this->getDietTargetDet($planId);
		$result['target_details'] = $arr_targetDetails;
		
		//echo '<pre>';print_r($result['guidelines']);exit;
	...
	public function getPlanProfile($planId) {
	...
	public function getDietGuidelines($planId) {
		$arr_diseases = array();
		$this->db->select('pg.disease_id,dr.recommendation,d.name,pg.checked,pg.recommend_id,pg.plan_id,pg.id');
		...
		foreach($result as $key=>$value) {
			...
			$value['chkd'] = ($value['checked']==1?'true':'false');
			$arr_diseases[$value['disease_id']]['drecommed'][] = $value;
		}
	...
	/* Function to clients Plans */
		public function getClients() {
			$this->db->select('pm.id,pm.first_name,pm.gender,pm.age,pm.file_no');
			$this->db->from('diet_plan_main as pm');
			$this->db->where(array('pm.status'=>'T'));
             ...
             foreach($result as $key=>$val)
				{
					...
					$res_plans[$i]['id']=$val['id'];
					$res_plans[$i]['first_name']=$val['first_name'];
					$res_plans[$i]['gender']=$val['gender'];
					$res_plans[$i]['age']=$val['age'];
					$res_plans[$i]['file_no']=$val['file_no'];
					
					
					$this->db->select('id as planid,DATE_FORMAT(consultation_date,"%d-%m-%Y") as plan_date');
					$this->db->from('diet_plan_profile');
					$this->db->where(array('client_id'=>$val['id']));
					$this->db->order_by('id desc');
					$queryp = $this->db->get();
	                $resultp = $queryp->result_array();
					foreach($resultp as $vv)
					{
						$res_plans[$i]['plans'][] = $vv;
					}
	    ...
	    public function getClientDtl() 
	    {
			...
			$query = $this->db->get();
				...
				$this->db->select('meal as id,meal_name as name,DATE_FORMAT(start_time,"%a %b %d %Y %H:%i:%s") as start_time,DATE_FORMAT(end_time,"%a %b %d %Y %H:%i:%s") as end_time,notes');
				...
				foreach($item_result as $key=>$val)
				{
					...
					$res_items[$val['id']]['aoptions'][$i][$j]['item'] = $val['item'];
					$res_items[$val['id']]['aoptions'][$i][$j]['exch'] = $val['exch'];
		...
		public function getFooditems() {
			...
			$this->db->where(array('im.status'=>'T'));
		...
		public function getClientIByPlanId($plan_id)
		{
			$this->db->select('client_id');
			$this->db->from('diet_plan_profile');
			$this->db->where(array('id'=>$plan_id));
       ...
       // Function to get general guidelines
		...
		public function getDietTargetDet($planId) {
			...
			$this->db->from('diet_plan_profile');
			$this->db->where(array('status'=>'T','id'=>$planId));
		...
		public function removeRecomend() {
			$post = $this->input->post();			
			$data['checked'] = $post['chk'];			
