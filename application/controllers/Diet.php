...
class Diet extends CI_Controller {
	...
	public function downloadPDF() {
		...
		$exercise_stat =  $this->uri->segment(5);
		$guideline_stat =  $this->uri->segment(6);
		$type =  $this->uri->segment(7);
		$data = $this->diet_model->getDietChartDetails($clientID,$planID);
		$data['exercise_stat'] = $exercise_stat;
		$data['guideline_stat'] = $guideline_stat;
		$data['type'] = $type;		
		...
		{
		    $this->load->view('plan/dietChart_pdf', $data);	
		}
	...
	public function sendMailChart() {
		...
		$planID =  $post['planId'];
		$exercise_stat =  $post['exercise_stat'];
		$guideline_stat =  $post['guideline_stat'];
		$type = $post['type'];
		$data = $this->diet_model->getDietChartDetails($clientID,$planID);
		...
		$mailto =  $data['main']['email']; //Mailto here
		$from_name = 'SelfCare'; //Name of sender mail
		$from_mail = 'mailfrom@selfcare.com'; //Mailfrom here
		$subject = 'Diet Chart'; 
		$message = "Hi ".$data['main']['first_name'].", Please find attached your diet chart";
		$filename = $data['main']['first_name']."-".date("d-m-Y_H-i",time()); //Your Filename with local date and time
	...
	public function removeRecomend() {
		$result = $this->diet_model->removeRecomend();  
		$this->output->set_content_type('application/json')->set_output(json_encode($result));
	}
